...
miniExtend(renderPrototype, {
    ...
    _handVary: function(varyOb, ob, item, blg) {//处理变量
      ...
      var defInd,defVal;
       ...
       // if(typeof tmpob==="object"){//如果tmpob是对象，
       //     tmpob=JSON.stringify(tmpob);
       // }
       return tmpob;
   },
    handVal: function(str, ob, item, blg) {
        ...
        if (filter && filter[0] === "EXPRESS") {
            ...
            if (expob.type === "for") {
                ...
                var scopeVal=expob[0];
                var scopeMatch=ts.regMatch().exec(scopeVal);
                if(scopeMatch){
                    scopeVal=scopeMatch[1];

                    scopeVal=ts._handVary(parseVariable(scopeVal),ob,item,blg);
